...
\begin{figure}
  \begin{center}
    ...
    \caption{\emph{Left:} Detections from an extended-source detector,
      ...
      ``yellow galaxy'' and ``yellow point source'' detectors.
      \emph{Right:} Relative strength of detection of sources in the
      extended versus PSF detection filters.  The extended-source
      filter used here is a 1-arcsecond FWHM round Gaussian.  There is
      a clear line of sources with a detection ratio around 0.95,
      corresponding to point sources.  Sources with ratios above unity
      are closer in profile to the target profile than to a point
      source, with the largest ratios probably corresponding to
      sources that are approximately equal to the target profile.
      \label{fig:gals}}
  \end{center}
\end{figure}
...
transient sources, including asteroids.  A few examples are shown in
\figref{fig:oneband}.
