...
Fundamentally, when much is understood about a signal latent in
...
\item the noise (coming from such sources as the Poisson distribution
  ...
  subtraction (sky estimation), while the constant-variance assumption implies
...
is computed by correlating the image with its PSF, evaluated on the original image pixel
...
\begin{figure}
    ...
    \end{center}
    \caption{The effect of the flux prior on the decision boundaries
...
\begin{figure}
  ...
  \caption{Decision boundaries in signal-to-noise space for the case
    ...
    Gaussian).  This illustrates how the $\chipos^2$ method improves upon
...
correspond to very red sources.  We use these bins as our SED-matched
...
In order to show the qualitative behavior of the Bayesian approach, we will
...
analytically, with a false-positive rate corresponding to $5 \sigma$ for a Gaussian.
