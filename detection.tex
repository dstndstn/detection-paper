...
rate, and therefore the number of true detected sources will be lower.


Although not mentioned in \cite{szalay1999}, one extension to their
method is to only count pixels with positive flux in the $\chi^2$
...
\end{eqnarray}
where $P_{\chi^2}(v^2, k)$ is the probability distribution function of
...
$k$ positive fluxes, and the $P_{\chi^2}$ term gives the distribution
...
\begin{figure}
  ...
  \begin{center}
    \includegraphics[width=0.5\textwidth]{chisq-pos-decision-boundary}
  \end{center}
  \caption{Decision boundaries in signal-to-noise space for the case
    of two bands ($g$ and $r$), and for two variants of the $\chi^2$
    and SED-matched detection methods.  For the $\chi^2$ detector, we
    show the traditional version (labelled), and the $\chipos^2$
    version defined in \eqnref{eqn:chipos}.  For the SED-matched
    detector, we show a version that takes the union of detections
    from three SEDs, and a Bayes version that sums the detection
    probabilities between thesame three SEDs.  The SEDs include
    $r$-only, $g=r-1$ (``red''), and $g=r$ (``flat'').  Pixels that
    lie above and to the right of these curves (or anywhere outside
    the $\chi^2$ circle) will be detected.  The thresholds have been
    set so that each method produces the same false-positive rate
    given pure Gaussian noise inputs.  This illustrates how the
    $\chipos^2$ method detects more true sources, thanks to treating
    negative fluxes differently than positive fluxes.  Notice that the
    SED-matched decision boundaries are shaped to have lower
    thresholds in directions that are consistent with the SEDs they
    are tuned to, and how the asymptotic behaviors in the $r$ and $g$
    directions are different because we chose (for purposes of
    illustration) to omit a $g$-only filter.
