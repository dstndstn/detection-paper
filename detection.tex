...
if we assume that our pixelized
model of the PSF is correct, then a Gaussian approximation typically
yields above $97\%$ efficiency.  A considerable side benefit of making
this assumption is that one can use a fast separable real-space
filtering routine to perform the correlation operation.
...
\emph{priors} so we expect will be slightly controversial.  We argue
...
generated, so it will be necessary to slightly increasing the
detection threshold to maintain false positives at an acceptable
level.  Since we effectively extract more of the available
...
\begin{figure}[p!]
  \begin{center}
    \begin{tabular}{cc}
      ...
      \includegraphics[width=0.39\textwidth]{bayes-only}
      %\hspace{0.03\textwidth}
      &
      \includegraphics[width=0.39\textwidth]{gri-only}
    \end{tabular}
    \caption{Bayesian SED-matched detection method compared to the
...
\begin{figure}[p!]
  \begin{center}
    \includegraphics[height=0.39\textwidth]{galaxies}
    \includegraphics[height=0.39\textwidth]{galaxies-relsn}
