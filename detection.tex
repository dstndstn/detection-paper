...
list of detected sources.  This is discussed below in section \ref{sec:thresholds}.
...
%       \multirow[t]{1}{0.45\textwidth}{
%       \framebox{\includegraphics[width=0.5\textwidth,height=0.4\textwidth]{dont-coadd}}%
%       }
%       &
%       \multirow[t]{1}{0.45\textwidth}{
%         text1 \\ text 2}
%       %\minitab[c]{text 1 \\ text 2 \\ text 3}
%       %}
%       %&
...
%   \begin{center}
%     \begin{tabular}{@{}c@{}c@{}c@{}}
%       %\multirow[c]{4}{*}[vmove-length]{%
%       \multirow[c]{4}{*}{%
%         % This figure comes from the dont-coadd.py script.
%         \rule{1pt}{0.4\textwidth}%
%         \includegraphics[width=0.5\textwidth]{dont-coadd}\bigstrut} &
%       Bad \\
%       & \parbox{0.45\textwidth}{\includegraphics[width=0.45\textwidth]{flow1} \\ testing \\ 1,2,3}\\
%       & Good \\
%       & \includegraphics[width=0.45\textwidth]{flow2} \\
%     \end{tabular}
%   \end{center}
%   \caption{The benefit of constructing a \emph{detection map} versus
%     \emph{coadding} the images and then detecting sources on the
...
%%% FIXME -- need to mention this plot in the text!!
...
%Figure \ref{fig:comp-images} illustrates this effect on real data:
%objects 
...
$gri$-union detections.  As shown in Figure \ref{fig:bayes-vs-gri},
...
% a large number of cosmic rays and
% asteroids, as well as a number of real very red sources.  If we clip
% outlier pixels while forming the detection maps, we can eliminate
% these cosmic rays before the detection step; the $gri$-union method
% then preferentially detects sources with extreme colors.  Since real
% images will often contain artifacts and moving objects, the fact that
% the $gri$-union method \emph{prefers} these objects to real sources is
% problematic; the Bayesian method, on the other hand, requires sources
% with such unexpected colors to have much higher signal than it does
% for real sources.
...
\begin{figure}[p!]
  \begin{center}
    \begin{tabular}{@{}c@{}c@{}c@{}c@{}c@{}c@{}}
      ...
      %
      % Transpose (note the caption also says above/below the diagonal
      %
      % \multirow[c]{5}{2em}[-0.45\textwidth]{\rotatebox[origin=l]{90}{Detected by method}} &
      % \multicolumn{5}{c}{Not detected by method}
      % \\
      % %
      % &
      % & Chi-squared $\chi^2$ & Chi-positive $\chipos^2$ & SED (union) & SED (Bayes) \\
...
\begin{figure}[p!]
  ...
  \begin{center}
    % I DO NOT KNOW WhyTF THIS IS NEEDED BUT OTHERWISE THE SPACE IS ALL FUCKED UP
    \hspace{-1.5cm}
    \begin{tabular}{@{}c@{}c@{}c@{}c@{}c@{}c@{}}
      %
      ...
      %
      % \multirow[c]{5}{2em}[-0.23\textwidth]{\rotatebox[origin=l]{90}{Detected by method}} &
      % \multicolumn{5}{c}{Not detected by method}
      % \\
      % &
      % & Chi-squared $\chi^2$ & Chi-positive $\chipos^2$ & SED (union) & SED (Bayes) \\
...
% SED union: total of 38 false detections
% [4, 3, 2, 1, 5, 6, 2, 6, 3, 6]
% SED mixture: total of 39 false detections
% [5, 2, 2, 4, 4, 3, 4, 7, 0, 8]
% Chi-sq: total of 59 false detections
% [4, 3, 1, 9, 6, 7, 7, 8, 8, 6]
% Chi-pos: total of 53 false detections
% [1, 6, 2, 5, 9, 5, 6, 6, 5, 8]
