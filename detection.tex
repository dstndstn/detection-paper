...
model in a statistical manner.  Setting this issue aside, we can see that
a given $p_\textrm{astro}$ threshold can be converted into a likelihood-ratio threshold:
\begin{eqnarray}
  \frac{p(\data \given \fg)}{p(\data \given \bg)} = \frac{p(\bg)}{p(\fg)} \frac{\pastro(\data)}{1 - \pastro(\data)}
\end{eqnarray}
where the priors also appear as in the Bayesian framework described above.
In this way, \pastro\ can be seen as an alternative way of expressing the subjective utilities in
