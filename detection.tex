...
grid than the original image pixels.  We will see below that when
combining multiple images, a \emph{resampling} step will be required
to bring all images to the same pixel grid; a similar step could be
used to interpolate the detection map of a single image.
Alternatively, we could lower our detection threshold to compensate
for the loss of detection efficiency due to sub-pixel offsets, then
fit for the best source position for each detection, dropping sources
whose best-fit values are below our original threshold.
In general, we should consider any detection procedure as generating
\emph{likely candidates} for further analysis, rather than being a final product.
