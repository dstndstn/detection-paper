\documentclass[11pt,letterpaper,linenumbers]{aastex7}
...
\newcommand{\secref}[1]{\mbox{Section~\ref{#1}}}
\newcommand{\subsecref}[1]{\mbox{Section~\ref{#1}}}
...
\newcommand{\sigd}{\sigma_{\scriptscriptstyle D}}
...
\newcommand{\pastro}{\ensuremath{p_\textrm{astro}}}
\newcommand{\hatpastro}{\ensuremath{\hat{p}_\textrm{astro}}}
...
%\correspondingauthor{dstndstn@gmail.com}
\author{David W. Hogg}
...
\begin{abstract}\noindent
  \changed{
  There is almost no data-analysis operation more important to astronomy than the detection of sources (stars or galaxies, say) in imaging.
  Here we write down a set of reasonable assumptions for well-understood (or well calibrated) background-dominated imaging (faint sources) and find the detection methods that flow from those assumptions.
  Our methods are hypothesis comparisons, involving matched filters.
  We show that they are generally preferable to one-hypothesis (p-value or n-sigma-deviation) methods, especially for avoiding spurious detection of non-star image features.
  We consider the case that there are multiple images at each point on the sky, and---more importantly for our purposes---when those images are taken through different bandpasses.
  Detection in multi-band imaging involves making choices about the range of colors or spectral energy distributions to which the method will be most sensitive; we deliver methods based on Bayesian decision theory and also frequentist methods that deliver similar outcomes in real-data tests.
  We discuss relationships between these methods and standard practices.
  The methods we present perform well, but our main point is that methods should be principled; that is, they should flow from our fundamental assumptions about the data.
  }
\end{abstract}
...
Fundamentally, when much is understood about a \changed{signal latent in
data with approximately Gaussian noise}, the best detection methods are (or look like) \emph{matched
filters}.
...
\citealt{zackay1} and \citealt{zackay2}); what is new is that we consider the common context of
...
program \citep[eg,][]{unwise-phot, lsoverview}.


\changed{
This article is structured as follows.
\Secref{sec:imagemodel} lays out the problem to be solved and the assumptions
we make.
\Secref{sec:detection} examines the single-image case, presenting the \emph{matched filter}
and introducing the notation we use, including defining the \emph{detection map}.
We comment on the ways that real data violate the assumptions we have made and how
this affects the results in \subsecref{sec:detectioncomments}.
We then describe the process of \emph{detecting} a point source as framed in various
statistical viewpoints in \subsecref{sec:thresholds}.
In \secref{sec:multiimage} we examine the multiple-image case, first for the case that
the images are all taken using the same bandpass filter.  This uses a straightforward
combination of the single-image detection maps.
Then, in \subsecref{sec:sedmatched} we examine the case of images taken through
different bandpass filters.  We introduce the \emph{SED-matched filter}; this is a novel
method and the primary contribution of this article.
In \subsecref{sec:bayessed} we give a Bayesian approach to marginalizing over SEDs,
which also requires specifying a prior over fluxes; we present one prior that leads to
tractable closed-form integrals.
In \secref{sec:chisq} we compare our new SED-matched filtering method with an existing
method, the ``Chi-squared coadd'' of \cite{szalay1999}.  This includes an extension to that
method that was suggested by our anonymous reviewer.
\Secref{sec:experiments} presents experimental results, first using simulated data to
compare the Chi-squared and SED-matched methods, then using real data from the
Dark Energy Camera \citep{decam}.
We conclude with a discussion in \secref{sec:discussion}.
}
...
\changed{Specifically, we will make the following strong assumptions:}
\begin{itemize}
\item the noise (coming from such sources as the Poisson distribution
  ...
  subtraction (sky estimation), \changed{while the constant-variance assumption implies
  that the Poisson variation of the source is dominated by other per-pixel uncertainties;}
...
% that one can use a fast (separable) real-space filtering routine
...
deviation $\psfw$ pixels has a norm approximately:
\begin{eqnarray}
...
list of detected sources.  This is discussed below in \secref{sec:thresholds}.


\subsection{Comments}
\label{sec:detectioncomments}
...
the matched filter; in \eqnref{eqn:psfdotprod}, the detection
...
is computed by correlating the image with its PSF, evaluated on the
\changed{
original image pixel
grid.  As the PSF becomes narrow, the detection efficiency varies
depending on the position of a source relative to the pixel center.  Essentially, the
correlation operation allows us to check many source hypotheses, \emph{with
the source centered in pixels.
}
The detection
...
\changed{
This effect can be reduced by evaluating the detection map on a finer
grid than the original image pixels.  We will see below
(in \secref{sec:multiimage}) that when combining multiple images, a
\emph{resampling} step will be required to bring all images to the
same pixel grid; a similar step could be used to interpolate the
detection map of a single image.  Alternatively, we could lower our
detection threshold to compensate for the loss of detection efficiency
due to sub-pixel offsets, then fit for the best source position for
each detection, dropping sources whose best-fit values are below our
original threshold.  In general, we should consider any detection
procedure as generating \emph{likely candidates} for further analysis,
rather than being a final product.
}
...
(including SourceExtractor, \citealt{sourceextractor}), there is a notion of requiring more than
...
exponential and de Vaucouleurs galaxy profiles \citep{gaussgals} are
convenient for this task, since they allow efficient separable
real-space correlation operations.


\subsection{Thresholds and Peaks}
...
\begin{eqnarray}
  \Lambda(\data) \; \triangleq \; \frac{p(\data \given H_1)}{p(\data \given H_0)} \; \overunderset{H_1}{H_0}{\gtrless} \; \eta
\end{eqnarray}
...
\begin{eqnarray}
  %\alpha = \int_{\Lambda(\data) > \eta}\displaylimits p(\data | H_0) \textrm{d,data}
  \alpha = \int_{\Lambda(x) > \eta}\displaylimits p(x \given H_0) \, \textrm{d}x
\label{eq:npthresh}
\end{eqnarray}
where for false positives, the data $x$ are truly drawn from the $H_0$
...
\citealt{astrometrydotnet}).  We must first write down our \emph{utility}
...
background models---which are related to the fraction of pixels that
truly contain stars---and the utilities, which describe how we are
willing to trade between different outcomes.

\changed{
Notice that as we increase our detection threshold, we will convert some false-positive
detections into true-negatives, changing the expected utility given in the
numerator in \eqnref{eq:kthresh}, while also converting some true-positive detections into
false-negatives, which changes the denominator.
}

Typically, in source detection we want to be relatively conservative:
false positive detections are considered to be quite bad, so we
set $u(\falsepos)$ to be a large negative number.  True positive
...
\changed{
\subsubsection{Probability of astrophysical event, \pastro}
In the gravitational wave community, the concept of \pastro\ is used
to characterize the probability that a given event is astrophysical, as opposed to being
due to detector noise or a terrestrial artifact.  This is defined (eg, in \citealt{unified_pastro}) as
\begin{eqnarray}
  \pastro(\data) = \frac{p(H_1 \given \data)}{p(H_1 \given \data) + p(H_0 \given \data)}
\end{eqnarray}
where $H_1$ is the hypothesis that the data are due to a real
astrophysical source, and $H_0$ is the hypothesis that the data are
due to an artifact or statistical noise.  Unlike the frequentist and
Bayesian frameworks described above, $H_0$ here includes
\emph{artifacts}, which are (almost by definition) exceedingly hard to
model in a statistical manner.  We will set this issue aside.
}
\changed{
We note that $H_1$ and $H_0$ constitute the only two hypotheses in
this model; the denominator in the above equation is therefore 1, and
we can see that $\pastro(\data)$ is simply a redefinition of $p(H_1
\given \data)$; the \emph{decision rule} is that we should claim a detection if
\begin{eqnarray}
  p(H_1 \given \data) > \hatpastro
\end{eqnarray}
where $\hatpastro$ is a scalar constant.
}
\changed{
Invoking Bayes's theorem, we can see that this can be rewritten as a likelihood-ratio test:
\begin{eqnarray}
  \frac{p(\data \given \fg) p(\fg)}%
       {p(\data \given \fg) p(\fg) + p(\data \given \bg) p(\bg)} &>& \hatpastro \\
       \frac{p(\data \given \fg)}{p(\data \given \bg)} &>& \frac{p(\bg)}{p(\fg)} \, \frac{\hatpastro}{1 - \hatpastro}
\end{eqnarray}
where the priors appear as in the Bayesian framework described above.
In this way, \hatpastro\ can be seen as an alternative way of expressing the subjective utilities in
a Bayesian decision theory setting.
}
...
flux is constant over time, and a series of images taken
with different noise levels, exposure
...
linear scaling (given our simple image model),
...
\paragraph{Coadds}
\changed{
Frequently, astronomers construct image coadds and then
detect sources by correlating the coadd with an estimate of its PSF
(eg, \citealt{stripe82coadd}).
It is straightforward to show that this is necessarily sub-optimal
unless the images have the same pixel-convolved PSF.  Intuitively, the PSF of the
coadd is not equal to the PSF of the individual images, so this approach is
using a ``mismatched filter'' rather than a matched filter, resulting
...
\begin{figure}[h]
  \begin{center}
    \begin{minipage}[c]{0.45\textwidth}
      \includegraphics[width=\textwidth]{dont-coadd}%
    \end{minipage}
    \begin{minipage}[c]{0.45\textwidth}
      \begin{center}
        % see Makefile - flow1.dot -> flow1.pdf
        Coadd then detect (loses information)\\
        \vspace{5pt}
      \includegraphics[width=\textwidth]{flow1}%
      \\
      \vspace{12pt}
      Combine detection maps (preserves information) \\
        \vspace{5pt}
...
map for each, using the correct coadded PSF.  As shown in \figref{fig:dontcoadd},
regardless of the coadd weight chosen, detecting
...
We can also view the problem as one of
...
sources of interest, and taking the union of sources detected.  By
...
\changed{
\begin{equation}
  p(\{ D_j \} \given \fg) = \iint p(\{ D_j \} \given \fg, F, s) \, p(F \given \fg) \,
  p(s \given \fg) \, \dd F \, \dd s
  \label{eq:margsed}
\end{equation}
where $F$ is the flux of the source in some canonical band, and $s$ is
the SED of the source.  $\fg$ indicates the model in which there is a source, and includes
any hyper-parameters in that model.  Here, we have made the strong assumption that the
flux and SED priors are independent.  We will assume that the SED prior $p(s \given \fg)$ is
...
\begin{eqnarray}
  p(\{ D_j \} \given \fg)
  &=&
  \sum_{i} w_i \int p(\{ D_j \} \given \fg, F, s_i) \, p(F \given \fg) \, \dd F
  \\
  &=&
  \sum_i w_i \int \prod_j \gaussian{D_j \given F \cdot s_{i,j}, \sigma_{D_j}^2} \, p(F \given \fg) \, \dd F
  \\
  &=&
  \sum_i w_i 
...
detection maps\footnote{The notation $\gaussian{x \given \mu, \sigma^2}$ indicates the likelihood
...
background distribution (\ie, the null hypothesis, $\bg$, that there is no
source)!

\changed{
Using an identity from \cite{gr1980},
% (specifically, it's 3.322-2 on page 307)
\begin{eqnarray}
...
\changed{
The reader would be forgiven for not recognizing that \eqnref{eq:pfg} is one of the
...
threshold at which to accept a peak in the $p(\{D_j\} \given \fg)/K$ map as a source can
...
\begin{figure}
  \begin{center}
    % These figures come from bayes_figure.py.
    \includegraphics[width=0.45\textwidth]{prob-contours-a}
    \includegraphics[width=0.45\textwidth]{prob-rel-a}
...
\begin{figure}
    ...
    \includegraphics[width=0.45\textwidth]{prob-contours-b}
    \includegraphics[width=0.45\textwidth]{prob-rel-b}
...
probability contours of the background model (\eqnref{eq:pbg})
and the foreground model (\eqnref{eq:pfg}), for the simplest
...
\begin{figure}
    \begin{center}
      % These figures come from bayes_figure.py.
      % matplotlib plot widths are 3.5 and 4.5 inches
      % -> ratio of 0.4375 to 0.5625
      \includegraphics[height=0.4\textwidth]{prob-contours-c}
      \includegraphics[height=0.4\textwidth]{prob-1d}
    \caption{The effect of the flux prior in Bayesian
      ...
      foreground model, \changed{as before, with $\alpha=1$, compared with a flux prior that
      ...
      fluxes, and therefore must produce smaller probabilities at low
...
\begin{figure}
    \begin{center}
    % This figure comes from chi_squared_experiment.py
    \includegraphics[width=0.45\textwidth]{alpha-det}
    \end{center}
    \caption{\changed{The effect of the flux prior on the decision boundaries
      of Bayesian SED-matched
      source detection.  The exponential scale factor $\alpha$ affects
      the shape of the decision contours.
      Decision contours for three
      different false-positive rates are shown ($10^{-1}$, $10^{-4}$
      and $10^{-7}$) for two different ``alpha'' values
      (``bright'': $\alpha = 1$ and ``faint'': $\alpha=4$).
      The foreground model is the three-SED model used in previous figures.
      %Notice that the largest ``alpha'' value (has
      %smoother decision contours.  For this largest ``alpha'', we also
      %plot larger false-positive rates of $10^{-11}$ and $10^{-17}$,
      %where the decision boundaries start to more closely follow the
      %shapes of the other curves, tracking the slopes of the
      %individual SED-matched filters.
      The detailed shape of the decision boundary is affected by the flux prior.
      }
    }
\end{figure}
...
\changed{
Here, since we are using detection maps, we need only consider
single peak pixels.
}
%See \figref{fig:sedmatched}.
...
distribution, which we call $\chipos^2$, with $d$ bands of data, then
\begin{eqnarray}
...
than the original $\chi^2$ detection filter, which therefore allows
...
\changed{
We first present comparisons between the original $\chi^2$ (``Chi-squared'') method, the
$\chipos^2$ (``Chi-positive'') variant presented above, and two variants of SED-matched
filtering, using synthetic data.  Then, we use real data from the Dark
Energy Camera \citep{decam}, first examining the behavior of the
SED-matched detection method, then comparing it with the $\chi^2$
methods.
}

\subsection{Chi-squared and SED-matched detection on synthetic data}
\label{sec:chisq_expt}

\begin{figure}
  % python chi_squared_experiment.py decision-boundaries
  \begin{center}
    \includegraphics[width=0.49\textwidth]{chisq-decision-boundary}%
    \hspace{0.01\textwidth}%
    \includegraphics[width=0.49\textwidth]{chisq-decision-boundary-sens}%
  \end{center}
  \caption{Decision boundaries in signal-to-noise space for the case
    of two bands ($g$ and $r$), and for two variants of the $\chi^2$
    and SED-matched detection methods.  \emph{Left:} In this case,
    the two bands are assumed to have equal levels of noise.  For the
    $\chi^2$ (Chi-squared) detector, we show the original version, and the
    $\chipos^2$ (Chi-positive) version defined in \eqnref{eqn:chipos}.  For the
    ...
    Gaussian).  This illustrates how the $\chipos^2$ method \changed{improves upon
    the $\chi^2$ method by detecting
    ...
    a $g$-only filter.  \emph{Right:} In this case, the noise in the
    ...
    signal in the $r$ band, and sculpt the decision boundaries
    accordingly.
  \label{fig:chipos_boundary}}
\end{figure}

\figref{fig:chipos_boundary} shows decision boundaries for the
...
plane.  The SED-matched methods have more complicated decision
...
The Bayesian SED-matched method has a similar decision boundary shape,
...
\begin{figure}
  \begin{center}
    % from chi_squared_experiment.py chisq-color
    \includegraphics[width=0.49\textwidth]{chisq-color}%
  \end{center}
...
\figref{fig:chipos_colors} shows the detection rates for the
...
\begin{figure}
  \begin{center}
    % from chi_squared_experiment.py chisq-det-color
    \includegraphics[width=0.55\textwidth]{chisq-det-colors}%
  \end{center}
...
\figref{fig:chipos_false} shows the colors of false detections
...
%%% FIXME --
% As shown in \figref{fig:sedmatched}, there is one additional
...
% We perform detections using
% each of the methods previously presented, with thresholds set to match the
% ``$5 \sigma$'' false positive rate for independent Gaussian noise.
% Detections are performed on a single exposure from each band,
% listed in \ref{tab:exposures}.  These exposures are from the 2016B semester,
% 
% For purposes of illustration, we have also produced a deep coadd of
% 100 exposures in each band.  This allows us to show which detections
% correspond to real objects in deeper imaging.
...
the DESI Legacy Surveys pipeline \citep{lsoverview, legacypipe},
...
The astrometric
calibration uses Gaia (DR2) as the reference catalog \citep{gaiaDR1,
  gaia}, and the photometric calibration uses the Pan-STARRS DR1
...
\begin{figure}
  \begin{center}
    \begin{tabular}{@{}c@{}c@{}c@{}}
      Blue-matched filter & Yellow-matched filter & Red-matched filter \\
      % from detection-figs.py
...
\begin{figure}
  \begin{center}
    ...
    \caption{Relative strengths of the different SED-matched detection
      filters with respect to star color.
      We match our detected sources with the Dark Energy
      ...
      strength 1.1 in the ``Blue'' filter would be detected at
      $5.5 \sigma$ in the ``Blue'' filter if it were detected at $5 \sigma$
...
\begin{figure}
  \begin{center}
    ...
    \begin{minipage}[c]{0.4\textwidth}
      \includegraphics[width=\textwidth]{bayes-vs-gri}%
    \end{minipage}%
    \begin{minipage}[c]{0.4\textwidth}
      \begin{center}
        Only detected by $gri$-union \\
        \includegraphics[width=\textwidth]{gri-only}\\
        Only detected by Bayesian \\
        \includegraphics[width=\textwidth]{bayes-only}\\
  ...
  \end{center}
    \caption{Bayesian SED-matched detection method compared to the
      union of $g$, $r$, and $i$ single-band detection.  \emph{Left:} The
      ...
      experiment, the detection thresholds were set very high and tuned so that both
      ...
      \emph{Right:} Examples of sources detected by only the $gri$-union or
      Bayesian methods.  The Bayesian-only sources have typical galaxy colors, while the
      $gri$-union-only sources tend to have very red or otherwise unusual colors.
      \label{fig:bayes-vs-gri}}
\end{figure}


\subsection{SED-matched detection on real data}
...
For the plots shown here, we select sources in the Yellow filter with
...
\subsection{Bayesian SED-matched detection on real data}
...
find 63 bins populated.  The prior
includes some SED components that have zero flux in $g$ band, which
correspond to very red sources.  \changed{We use these bins as our SED-matched
filters, with weights defined by the number of objects per bin.
}

We run the Bayesian detection procedure described in
\eqnref{eq:pfg}, computing in log space to avoid numerical overflow.
This takes a few minutes on a single core with our unoptimized
implementation.

In order to show the \changed{qualitative behavior} of the Bayesian approach, we will
...
$gri$-union detections.  As shown in \figref{fig:bayes-vs-gri},
the sources detected by only the Bayesian method
...
\begin{figure}[p!]
  \begin{center}
    \begin{tabular}{@{}c@{}c@{}c@{}c@{}c@{}c@{}}
      \multirow[c]{5}{2em}[-0.35\textwidth]{\rotatebox[origin=l]{90}{Not detected by method (except on diagonal)}} &
      \multicolumn{5}{c}{Detected by method}
      \\
      % from detection-figs.py
      &
      ...
      \includegraphics[height=0.23\textwidth]{chisq-images} &
      \includegraphics[height=0.23\textwidth]{unmatched-chipos-chisq-2} &
      \includegraphics[height=0.23\textwidth]{unmatched-sed-union-chisq-2} &
      \includegraphics[height=0.23\textwidth]{unmatched-sed-mix-chisq-2} \\
      %
      %Chi-positive &
      &
      \raisebox{0.05\textwidth}{\rotatebox[origin=l]{90}{Chi-positive $\chipos^2$}} &
      \includegraphics[height=0.23\textwidth]{unmatched-chisq-chipos-2} &
      \includegraphics[height=0.23\textwidth]{chisq-pos-images} &
      \includegraphics[height=0.23\textwidth]{unmatched-sed-union-chipos-2} &
      \includegraphics[height=0.23\textwidth]{unmatched-sed-mix-chipos-2} \\
      %
      %SED union &
      &
      \raisebox{0.05\textwidth}{\rotatebox[origin=l]{90}{SED (union)}} &
      \includegraphics[height=0.23\textwidth]{unmatched-chisq-sed-union-2} &
      \includegraphics[height=0.23\textwidth]{unmatched-chipos-sed-union-2} &
      \includegraphics[height=0.23\textwidth]{sed-union-images} &
      \includegraphics[height=0.23\textwidth]{unmatched-sed-mix-sed-union-2} \\
      %
      %SED mixture &
      &
      \raisebox{0.05\textwidth}{\rotatebox[origin=l]{90}{SED (Bayes)}} &
      \includegraphics[height=0.23\textwidth]{unmatched-chisq-sed-mix-2} &
      \includegraphics[height=0.23\textwidth]{unmatched-chipos-sed-mix-2} &
      \includegraphics[height=0.23\textwidth]{unmatched-sed-union-sed-mix-2} &
      \includegraphics[height=0.23\textwidth]{sed-mix-images} \\
    \end{tabular}
  \end{center}
  \caption{Sources detected by the different methods.  On the diagonal
    ...
    first (brighest) and last (faintest) 18 sources.  Notice that the brightest
    ...
    For the images below the diagonal, a significant number of the additional
    ...
    In general, the SED-matched methods produce more real source detections
    than the Chi-squared methods.
...
%   \begin{center}
%     \begin{tabular}{ccccc}
%       & Chi-squared & Chi-positive & SED union & SED mixture \\
%       %Chi-squared &
%       \raisebox{0.05\textwidth}{\rotatebox[origin=l]{90}{Chi-squared}} &
%       & %\includegraphics[height=0.24\textwidth]{chisq-images} &
%       \includegraphics[height=0.24\textwidth]{unmatched-chisq-chipos-4} &
%       \includegraphics[height=0.24\textwidth]{unmatched-chisq-sed-union-4} &
%       \includegraphics[height=0.24\textwidth]{unmatched-chisq-sed-mix-4} \\
%       %
%       %Chi-positive &
%       \raisebox{0.05\textwidth}{\rotatebox[origin=l]{90}{Chi-positive}} &
%       \includegraphics[height=0.24\textwidth]{unmatched-chipos-chisq-4} &
%       & %\includegraphics[height=0.24\textwidth]{chisq-pos-images} &
%       \includegraphics[height=0.24\textwidth]{unmatched-chipos-sed-union-4} &
%       \includegraphics[height=0.24\textwidth]{unmatched-chipos-sed-mix-4} \\
%       %
%       %SED union &
%       \raisebox{0.05\textwidth}{\rotatebox[origin=l]{90}{SED union}} &
%       \includegraphics[height=0.24\textwidth]{unmatched-sed-union-chisq-4} &
%       \includegraphics[height=0.24\textwidth]{unmatched-sed-union-chipos-4} &
%       & %\includegraphics[height=0.24\textwidth]{sed-union-images} &
%       \includegraphics[height=0.24\textwidth]{unmatched-sed-union-sed-mix-4} \\
%       %
%       %SED mixture &
%       \raisebox{0.05\textwidth}{\rotatebox[origin=l]{90}{SED mixture}} &
%       \includegraphics[height=0.24\textwidth]{unmatched-sed-mix-chisq-4} &
%       \includegraphics[height=0.24\textwidth]{unmatched-sed-mix-chipos-4} &
%       \includegraphics[height=0.24\textwidth]{unmatched-sed-mix-sed-union-4} &
%       %\includegraphics[height=0.24\textwidth]{sed-mix-images} \\
%     \end{tabular}
%   \end{center}
%   \caption{Sources detected by one method and not another.
%     For each source, a vertical slice through the detection map in each band is
%     plotted in signal-to-noise units.
%     Up to 20 randomly selected sources are plotted.
%     }
% \end{figure}
...
\begin{figure}[p!]
  \begin{center}
    \begin{tabular}{@{}c@{}c@{}c@{}c@{}c@{}c@{}}
      ...
      &
      \raisebox{0.05\textwidth}{\rotatebox[origin=l]{90}{Chi-squared $\chi^2$}} &
      \includegraphics[height=0.23\textwidth]{colorcolor-chisq} &
      \includegraphics[height=0.23\textwidth]{unmatched-chipos-chisq-7} &
      \includegraphics[height=0.23\textwidth]{unmatched-sed-union-chisq-7} &
      \includegraphics[height=0.23\textwidth]{unmatched-sed-mix-chisq-7} \\
      %
      %Chi-positive
      &
      \raisebox{0.05\textwidth}{\rotatebox[origin=l]{90}{Chi-positive $\chipos^2$}} &
      \includegraphics[height=0.23\textwidth]{unmatched-chisq-chipos-7} &
      \includegraphics[height=0.23\textwidth]{colorcolor-chipos} &
      \includegraphics[height=0.23\textwidth]{unmatched-sed-union-chipos-7} &
      \includegraphics[height=0.23\textwidth]{unmatched-sed-mix-chipos-7} \\
      %
      %SED-union
      &
      \raisebox{0.05\textwidth}{\rotatebox[origin=l]{90}{SED (union)}} &
      \includegraphics[height=0.23\textwidth]{unmatched-chisq-sed-union-7} &
      \includegraphics[height=0.23\textwidth]{unmatched-chipos-sed-union-7} &
      \includegraphics[height=0.23\textwidth]{colorcolor-sed-union} &
      \includegraphics[height=0.23\textwidth]{unmatched-sed-mix-sed-union-7} \\
      %
      %SED-mixture
      &
      \raisebox{0.05\textwidth}{\rotatebox[origin=l]{90}{SED (Bayes)}} &
      \includegraphics[height=0.23\textwidth]{unmatched-chisq-sed-mix-7} &
      \includegraphics[height=0.23\textwidth]{unmatched-chipos-sed-mix-7} &
      \includegraphics[height=0.23\textwidth]{unmatched-sed-union-sed-mix-7} &
      \includegraphics[height=0.23\textwidth]{colorcolor-sed-mix} \\
    \end{tabular}
  \end{center}
...
\begin{figure}[p!]
  \newcommand{\rheader}[1]{\rotatebox{90}{#1}}
  \begin{center}
    % I DO NOT KNOW WhyTF THIS IS NEEDED BUT OTHERWISE THE SPACE IS ALL BORKED UP
    \hspace{-1.5cm}
    %
    \begin{tabular}{@{}c@{}c@{}c@{}c@{}c@{}c@{}}
      %
      \multirow[c]{5}{2em}[-0.13\textwidth]{\rotatebox[origin=l]{90}{Not detected by method (except for diagonal}} &
      \multicolumn{5}{c}{Detected by method}
      \\
      % from detection-figs.py
      &
      ...
      % Chi-sq
      &
      \rheader{Chi-squared $\chi^2$} &
      \includegraphics[width=0.12\textwidth]{median-chisq} &
      \includegraphics[height=0.12\textwidth]{unmatched-chipos-chisq-9} &
      \includegraphics[height=0.12\textwidth]{unmatched-sed-union-chisq-9} &
      \includegraphics[height=0.12\textwidth]{unmatched-sed-mix-chisq-9} \\
      %
      %Chi-positive
      &
      \rheader{Chi-positive $\chipos^2$} &
      \includegraphics[width=0.12\textwidth]{unmatched-chisq-chipos-9} &
      \includegraphics[height=0.12\textwidth]{median-chipos} &
      \includegraphics[height=0.12\textwidth]{unmatched-sed-union-chipos-9} &
      \includegraphics[height=0.12\textwidth]{unmatched-sed-mix-chipos-9} \\
      %
      % SED union
      &
      \rheader{\hspace{1em}SED (union)} &
      \includegraphics[width=0.12\textwidth]{unmatched-chisq-sed-union-9} &
      \includegraphics[height=0.12\textwidth]{unmatched-chipos-sed-union-9} &
      \includegraphics[height=0.12\textwidth]{median-sed-union} &
      \includegraphics[height=0.12\textwidth]{unmatched-sed-mix-sed-union-9} \\
      %
      %SED mixture
      &
      \rheader{\hspace{1em}SED (Bayes)} &
      \includegraphics[width=0.12\textwidth]{unmatched-chisq-sed-mix-9} &
      \includegraphics[height=0.12\textwidth]{unmatched-chipos-sed-mix-9} &
      \includegraphics[height=0.12\textwidth]{unmatched-sed-union-sed-mix-9} &
      \includegraphics[height=0.12\textwidth]{median-sed-mix} \\
    \end{tabular}
  \end{center}
  \caption{Sources detected by each method.  On the diagonal, we show
    ...
    method, again the median in a deep coadd.  In general, the images above
...
\begin{figure}[p!]
  \begin{center}
      % from detection-figs.py
    \includegraphics[height=0.35\textwidth]{galaxies}
    \includegraphics[height=0.35\textwidth]{galaxies-relsn}
...
\subsection{Comparing $\chi^2$ and SED-matched methods on real data}
...
the $g$, $r$, and $i$ bands: DECam exposure numbers 563982, 569613, 567442, respectively.

Thresholds for the $\chi^2$ and $\chipos^2$ methods were set
analytically, \changed{with a false-positive rate corresponding to $5 \sigma$ for a Gaussian.}
The SED-matched ones were set empirically by drawing
...
our experiments on synthetic data in \secref{sec:chisq_expt}: the
...
\figref{fig:comp-images} shows images of the sources that are detected
by one method and not by each other method.
\figref{fig:comp-cc} also compares pairs of methods, showing the
color-color space distributions of the detections.
\figref{fig:comp-median-image} further compares pairs of methods, showing
...
\changed{
If a project wishes to avoid either Bayesian statistical philosophy or else any
computational demands of marginalization, the SED union methodology
is frequentist-safe and requires no marginalizations.
Our experiments suggest that the SED Bayes and SED union methods are
expected to deliver very similar catalogs or results in most settings.
Technically, the SED union methodology is effectively a frequentist profile
over the family of permitted SEDs.
}
...
% --- this would allow us to use a broad range of filters and source
% models, and detect different classes of objects simultaneously in
% one framework.

\changed{
Source detection is often one of the first steps in an astrophysical
data analysis.  If a Bayesian method is used to detect sources, then
care must be taken in the interpretation of these sources; the exact
set of sources detected will be affected by the priors.  Of course,
the same issue will occur with more heuristic approaches such as
taking the union of detections in individual bandpasses, or using a
method such as the Chi-squared coadd; combining information from
multiple bandpasses necessarily leads to sensitivity that varies with
source SEDs.  With a Bayesian method, the priors are explicit and can be
correctly incorporated into downstream analyses.
}

\changed{
In our Bayesian version of the SED-matched filter, we assumed that the
SED and flux priors are independent.  It would be more realistic to
assume that these are coupled; blue quasars and red galaxies have
different luminosity functions.  This would require specifying a flux
prior for each SED under consideration, considerably expanding the
number of priors required and perhaps making interpretation of the
results more difficult.
}

% Real astronomical images contain signal in some pixels due to
% artifacts such as cosmic rays, as well as rapidly-moving objects such
% as asteroids or Earth satellites.  In the experiments presented here,
% these pixels have been masked using a heuristic outlier-rejection step
% in the \emph{legacypipe} software.

\changed{
In our multi-image case, we assumed that the \emph{calibration} of
each images was perfectly known.  In real images, this is something
that must be measured, and some of the most precise calibrations use
\emph{self-calibration} or \emph{\"uber-calibration} approaches
\citep{ubercal}, leveraging sources detected in multiple images to
determine their relative calibration.  In a fully self-consistent
approach, this would tie together calibration and source detection in
a way that would make sources not independent of each other.  In
practice, these approaches are typically iterative rather than fully
self-consistent, and tend to use brighter, high-signal-to-noise stars
as cross-calibrators, so assumptions of independence at source
detection time is reasonable.
}
...
\eqnref{eq:detmap1}, we have found that the map $M_{\jvec}$ can
be computed by correlating the image with its PSF:
\begin{equation}
...
\eqnref{eq:onebandmap} saturates the Cram\'er--Rao bound for $F$.
...
%     \end{tabular}
%     \end{scriptsize}
%     \caption{Exposures from the Dark Energy Camera used for detection in
%       the experiments.\label{tab:exposures}}
%   \end{center}
% \end{table}
