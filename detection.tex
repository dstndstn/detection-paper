...
band as described above, and run several SED filters: a ``red'' filter
matched to a source with colors $g - r = r - i = 1$; a ``flat'' filter
with colors $g-r = r-i = 0$; and,  a ``blue'' filter with colors $g-r =
r-i = -1$.  (In the AB magnitude system here, very few real
astrophysical sources have colors bluer than zero; 
