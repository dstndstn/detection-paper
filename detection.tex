...
detection of point sources in astronomical images.  We consider only the
case of an isolated point source in
background-dominated imaging with perfectly known background level,
point-spread function, and noise models; this is an idealized
limit of typical optical astronomical images.
%
% We show that the matched
...
% optimal multi-image point source detection map.
%
We review the extension from single-image to multi-image detection, where
the images have different noise levels and point-spread functions but
identical bandpass filters.
%
We then extend the approach to
...
through different filters, but requires us to specify
the colors of the objects to which we wish to be most sensitive.
%
We show that this approach is superior
to some methods traditionally employed,
and that other traditional
...
%
We present approaches that are justified in terms of frequentism, Bayesian decision theory,
and standard astronomical practices; all three are very similar from an operational perspective.
We illustrate our results with experiments on synthetic and real data.
% (if I mention DECam do I need to reference it?  references in abstracts are annoying)
\end{abstract}
