...
We must now specify a prior over the flux to make progress.  A
not-unreasonable choice that leads to a relatively tractable result is
an exponential prior, $p(F) = \alpha \exp(-\alpha F)$ and $F >
0$, with $\alpha$ a constant to be chosen.  For the purposes of source
detection, where we will be competing this ``foreground'' model for a
source against a ``background'' (noise) model, this flux prior (with
$\alpha = 1$) has little influence; the likelihoods dominate.  With
this flux prior, we have
\begin{eqnarray}
  p_S(\{ D_j \})
  &=&
  K \alpha
...
%% FIXME -- can we do it analytically?  Should be able to!!
...
\begin{figure}
    ...
    \caption{ \emph{Left:} likelihood contours for the background
      ...
      ``red'' (color $g-r=1$ mag), 49\% ``flat'' ($g=r$), and 2\%
      $r$-band only ($g=0$).  Note that the contours have components
      inclined in the direction of the SED components (shown by faint
      lines).  The contours are at powers of 10.  \emph{Right:}
      likelihood ratio contours for the foreground divided by
      background model.  The contours are at powers of 10.
      \label{fig:conb}
    }
    \end{center}
\end{figure}
...
case where our SED prior $p(s)$ is a delta function at
$s_{g} = \frac{1}{3.5}$, $s_{r} = \frac{2.5}{3.5}$: this is a ``red'' detection
filter that assumes a $g - r$ color of 1 magnitude.



\paragraph{You always have a prior} And if you don't think you have a prior,
you're probably using a wrong prior.
