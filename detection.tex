...
to some methods traditionally employed, such as the \emph{chi-squared
  coadd}, and that other traditional methods can be seen as instances
of SED-matched filtering with implied (and often unreasonable) priors.
...
% In this \doctype\ we use the word
% ``optimal'' in the technical sense of saturating the Cram\'er--Rao bound
% under stated assumptions (and we encourage other authors to do the
% same).
...
% In the "image model" section you don't mention the PSF.  We assume:
% - the point source is convolved with a pixel-convolved PSF and added into the image linearly
% - the pixel-convolved PSF is known very precisely
% These assumptions are extreme and required for any claim of optimality (see footnote!).  In general, this "image model" section could state all of our assumptions clearly (almost as a bulleted list -- or actually).
% 
...
proportional to the number of photons incident upon it during the
exposure, modulated by a bandpass filter.  We will consider a discrete
image made up of a square array of pixels $\jvec$, each of which has
signal (or counts) $C_{\jvec}$, where we use $\jvec$ as a
two-dimensional focal-plane position, measured in integer pixel units.

% We will assume that the noise (coming from such sources as the Poisson
% distribution of the sky background, dark current, and read noise) is
% zero-mean, Gaussian, pixelwise independent, and of known variance.
% This assumption is not too bad for sky-limited images from which the
% sky level has been subtracted (for example, typical optical or
% near-infrared images).  We assume that we can estimate the sky level
% exactly; in reality our estimate will be noisy.  We will ignore the
% fact that the signal itself likely follows a Poisson distribution.  We
% will assume that the image is well sampled.

We will make many strong assumptions:
\begin{itemize}
\item that the noise (coming from such sources as the Poisson
  distribution of the sky background, dark current, and read noise) is
  zero-mean, Gaussian, pixelwise independent, and of known variance.
  The zero-mean assumption can be seen as assuming perfect background
  subtraction (sky estimation);
  %This assumption is not too bad for sky-limited images from which the
  %sky level has been subtracted (for example, typical optical or
  %near-infrared images)
\item that the image is well sampled;
%\item that the image contains continuous (not integer) 
\item that the (perhaps tiny) image contains \emph{only} one point
  source with some (unknown) flux and position;
\item that the source is centered within a pixel (we will relax this
  later);
\item that the point-spread function is spatially constant (across the
  possibly small image patch of interest) and known perfectly;
\item that the photometric calibration of the image is known
  perfectly; that is, that it is possible to map from ``count'' units
  back to physical units or a photometric standard;
\item that the image is not contaminated by cosmic rays, stray light,
  bad pixels, bad columns, electronic artifacts, or any other of the
  many defects in real images.
\end{itemize}
Throughout this \doctype\ we assume a ``pixel-convolved'' PSF;%
\footnote{Which should probably be called the ``pixel-correlated''
  PSF!}  we consider the point-spread function to \emph{include} the
effects of pixelization.  In well-sampled images, we think of the
image as being a continuous function which, after being correlated by
the PSF, is sampled at delta-function pixel locations.  There is no
need to think of pixels as ``little boxes''; they are simply samples
of an underlying smooth two-dimensional signal.
...
%
Specifically, if the image contains a single point source, centered on
...
from a zero-mean Gaussian with known per-pixel variance $\sigma_1^2$.
%
...
% Put simply, in our model the image pixels contain signal due to a
% pixel-convolved point-spread function $\psi(\cdot)$ shifted to the
% source center $\kvec$ and scaled by the source counts $C_k$; plus
% additive, pixelwise-independent, constant-variance Gaussian noise.
...
% For simplicity, we have dropped all
% assumption-violating contributions from galaxies, nebulosity,
% neighboring sources, or sky gradients.
...
the PSF'' or ``convolving by the PSF'' can be written as
\begin{eqnarray}
...
signal that is dispersed into many pixels by the PSF.  In
\appref{app:lindet} we derive the matched filter and 
...
FIXME -- the paragraph below is repetitive.
...
\begin{equation}
%  \signoise_{D_{\jvec}} = \frac{C_k \, \sum_{\iina} a_{\ivec} \, \psfat(\ivec + \jvec - \kvec)}{\sigma_1 \sqrt{\sum_{\iina} a_{\ivec}^2}}
...
$\ivec$, we have:
\begin{eqnarray}
  %\signoise_{D_{\jvec}} &=& \frac{C_k \, \avec \cdot \bm{\psfat(\jvec-\kvec)}}{\sigma_1 \sqrt{\avec \cdot \avec}} \\
