...
% -- go very carefully through the text and appendices, checking for
%    consistency in notation -- eg C_j vs S_j, ``flux'' vs ``source counts''

% -- reference for brighter-fatter?
...
% Are we going to advocate iterating over a set of SEDs, or going
% Bayesian?  Are they much different?

%% Use ``s'' for SED-matched filter SED rather than f_j.
...
a fluctuation in the image noise.

The likelihood for a single pixel in the set of detection maps $d_j$,
given the existence of a source, is
\begin{equation}
...
\begin{eqnarray}
  ...
  \int 
    ...
    \, p(f) \, \dd f
\end{eqnarray}
where we have written out the detection map's Gaussian likelihood.
...
an exponential prior, $p(f) \propto \alpha \exp(-\alpha f)$ and $f >
0$.  For the purposes of source detection, where we will be competing
this ``foreground'' model for a source against a ``background''
(noise) model, the flux prior has little influence; the likelihoods
dominate.  With this flux prior, we have
\begin{eqnarray}
  p_S(\{ d_j \})
  &=&
  \sum_i w_i 
